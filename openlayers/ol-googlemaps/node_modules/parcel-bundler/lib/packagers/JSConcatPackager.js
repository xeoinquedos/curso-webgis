...
const getName = _require.getName,
      getIdentifier = _require.getIdentifier;


const prelude = {
  source: fs.readFileSync(path.join(__dirname, '../builtins/prelude2.js'), 'utf8').trim(),
  minified: fs.readFileSync(path.join(__dirname, '../builtins/prelude2.min.js'), 'utf8').trim().replace(/;$/, '')
};

const helpers = fs.readFileSync(path.join(__dirname, '../builtins/helpers.js'), 'utf8').trim() + '\n';
...
class JSConcatPackager extends Packager {
  ...
  markUsed(mod, name) {
    var _findExportModule = this.findExportModule(mod.id, name);

    let id = _findExportModule.id;
  ...
  addBundleLoader(bundleType, parentAsset, dynamic) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (sourceMaps) {
        ...
        if (mapBundle) {
          output += `\n//# sourceMappingURL=${urlJoin(_this5.options.publicURL, path.basename(mapBundle.name))}`;
        }
      }
  ...
  findExportModule(id, name, replacements) {
    ...
    let exp = asset && Object.prototype.hasOwnProperty.call(asset.cacheData.exports, name) ? asset.cacheData.exports[name] : null;
