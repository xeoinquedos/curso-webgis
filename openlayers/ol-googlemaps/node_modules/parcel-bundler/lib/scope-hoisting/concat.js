...
const getName = _require2.getName,
...
module.exports = (packager, ast) => {
  ...
  function replaceExportNode(module, originalName, path) {
    var _packager$findExportM = packager.findExportModule(module.id, originalName, replacements);

    let identifier = _packager$findExportM.identifier,
  ...
  function interop(mod, originalName, path, node) {
    ...
    if (mod.cacheData.isCommonJS && originalName === 'default') {
      ...
      if (!path.scope.getBinding(name)) {
        var _path$getStatementPar = path.getStatementParent().insertBefore(DEFAULT_INTEROP_TEMPLATE({
        ...
        if (t.isObjectPattern(id)) {
          ...
          try {
            for (var _iterator3 = path.get('id.properties')[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
              ...
              var _packager$findExportM2 = packager.findExportModule(match[1], key.name, replacements);

              let identifier = _packager$findExportM2.identifier;
    ...
    MemberExpression: {
      exit(path) {
        ...
        if (match) {
          ...
          var _packager$findExportM3 = packager.findExportModule(match[1], name, replacements);

          let identifier = _packager$findExportM3.identifier;
